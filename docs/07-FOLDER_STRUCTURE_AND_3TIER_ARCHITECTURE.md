...
5. **Rule 5 (Automated Architecture Fitness Functions - [ADR-0012](adr/ADR-0012-architecture-fitness-testing-with-pytest-ast.md)):** Layer boundary compliance is statically verified on every CI run via `backend/tests/test_architecture_fitness.py`. Using Python's native AST parser, the test suite verifies that presentation controllers never import repositories or DB pools, services remain 100% framework-independent, core primitives never have circular dependencies, schemas remain pure DTOs, and domain services adhere to ADR-0010 (Result Pattern).
